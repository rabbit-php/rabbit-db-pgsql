...
WHERE ns.nspname = ? AND c.relkind IN ('r','v','m','f', 'p')
ORDER BY c.relname
SQL;

        return $this->db->createCommand($sql, [$schema])->queryColumn();
    }
    /**
     * @author Albert <63851587@qq.com>
     * @param string $name
...
WHERE "tcns"."nspname" = ? AND "tc"."relname" = ?
...
SQL;
        ...
        foreach ($indexes as $name => $index) {
            ...
            $ic->isPrimary = (bool) $index[0]['index_is_primary'];
            $ic->isUnique = (bool) $index[0]['index_is_unique'];
...
FROM pg_class c
INNER JOIN pg_namespace ns ON ns.oid = c.relnamespace
WHERE ns.nspname = ? AND (c.relkind = 'v' OR c.relkind = 'm')
ORDER BY c.relname
SQL;
...
SQL;
        ...
        foreach ($constraints as $type => $names) {
            foreach ($names as $name => $constraint) {
                switch ($type) {
                    case 'p':
                        $ct = (new Constraint())->name($name)->columnNames(ArrayHelper::getColumn($constraint, 'column_name'));
